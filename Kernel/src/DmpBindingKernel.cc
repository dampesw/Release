...
BOOST_PYTHON_MODULE(libDmpKernel){
  ...
  // DmpGeometryManager
  class_<DmpGeometryManager,boost::noncopyable,bases<DmpVSvc> >("DmpGeoMgr",init<>())
  //class_<DmpGeometryManager,boost::noncopyable>("DmpGeoMgr",no_init)
    //.def("GetMainGdmlName", &DmpGeometryManager::getGeometryMainGdmlName,return_value_policy<reference_existing_object>())
    //.def("SetMainGdmlName", &DmpGeometryManager::SetGeometryMainGdmlName)
    //.staticmethod("SetMainGdmlName")
  ;
